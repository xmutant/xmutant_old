...
export default function EditProfile() {
  ...
  const {
    ...
    isUploading,
    error: uploadError,
  } = useFileUpload(process.env.NEXT_PUBLIC_LIGHTHOUSE_KEY);
  ...
  const handleProfileImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        setImage(reader.result);
        setFormData({ ...formData, image: file }); // Update formData with the selected file
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleCoverImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        setFormData({ ...formData, coverImage: file }); // Update formData with the selected file
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    await uploadFiles([formData.image]);
    console.log("profile URLs::::::", updateProfileUrls[0]);

    const updatedFormData = await {
      ...formData,
      profileImage: uploadedFileUrls[0],
      // coverImage: uploadedFileUrls[1],
    };
