...
export function hexToRgba(hexCode) {
  ...
  const a =
    Math.round(parseInt(hex.substring(6, 8), 16) / 255 + Number.EPSILON) / 100;
...
export const ParameterProcessors = {
  number: {
    serialize: (input) => {
...
export function serializeParams(params, definition) {
...
export function serializeParamsOrNull(params, definition) {
...
export function deserializeParams(bytes, definition, options) {
  const params = {};
  for (const def of definition) {
...
export function consolidateParams(definition, data) {
 ...
 */
export function buildParamsObject(definition, data) {
  if (!definition) return {};
...
export function getRandomParamValues(params, options) {
...
export function sumBytesParams(definitions) {
  return (
    definitions?.reduce(
...
export function stringifyParamsData(data) {
  return jsonStringifyBigint(data);
}

export function jsonStringifyBigint(data) {
...
export function fxParamsAsQueryParams(snippetVersion) {
  if (snippetVersion === "") return false;
  return !!(
    semver.valid(snippetVersion) && semver.lte(snippetVersion, "3.2.0")
  );
}
